...
import type { CaseStatus } from '@/data/mock-dashboard'

const statusStepCount: Record<CaseStatus, number> = {
  broker_assigned: 1,
  seller_contacted: 2,
  buyer_contacted: 3,
  explanation_done: 4,
...
const summaryCards = [
  { label: '全案件', value: mockCases.length, icon: Clock, color: 'text-foreground' },
  { label: '進行中', value: mockCases.filter((c) => !['settlement_done', 'cancelled'].includes(c.status)).length, icon: AlertCircle, color: 'text-info-500' },
  { label: '決済完了', value: mockCases.filter((c) => c.status === 'settlement_done').length, icon: CheckCircle, color: 'text-success-500' },
  { label: '中止', value: mockCases.filter((c) => c.status === 'cancelled').length, icon: XCircle, color: 'text-error-500' },
]
    ...
    >
      ...
      <div className="bg-white rounded-2xl shadow-card p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select className="px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-500/20">
            <option value="">全ステータス</option>
            {Object.entries(CASE_STATUS_LABEL).map(([key, label]) => (
              <option key={key} value={key}>{label}</option>
            ))}
          </select>
          <button className="px-5 py-2.5 text-sm font-medium text-white bg-primary-500 rounded-xl hover:bg-primary-600 transition-colors">
            検索
          </button>
        </div>
      </div>
      ...
      <div className="hidden lg:block bg-white rounded-2xl shadow-card">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {mockCases.map((c) => {
                const step = statusStepCount[c.status]
                ...
                return (
                  <tr key={c.id} className="border-t border-neutral-100 hover:bg-neutral-50/50">
                    ...
                    <td className="py-3.5 px-5 text-right price">{c.amount.toLocaleString()}<span className="text-xs font-normal text-neutral-400 ml-0.5">万円</span></td>
                    ...
                    <td className="py-3.5 px-5 text-xs text-neutral-400">{c.updatedAt}</td>
                    <td className="py-3.5 px-5">
                      <Link href={`/broker/cases/${c.id}`} className="text-primary-500 hover:underline text-xs font-medium">
                        詳細
                      </Link>
                    </td>
                  </tr>
      ...
      <div className="lg:hidden space-y-3">
        {mockCases.map((c) => {
          ...
          const progressPct = c.status === 'cancelled' ? 0 : (step / 6) * 100
          return (
            <Link key={c.id} href={`/broker/cases/${c.id}`} className="block bg-white rounded-2xl shadow-card p-4 hover:shadow-md transition-shadow">
              ...
              <div className="grid grid-cols-2 gap-1 text-xs text-neutral-400 mb-3">
                ...
                <p>成約額: <span className="price text-neutral-600">{c.amount.toLocaleString()}</span>万円</p>
              </div>
